...
TOPICS = [
    # 가전/IT
    {"key": "airfryer_review",          "title": "에어프라이어 추천 TOP 5 — 쿠팡 평점순 비교 가이드",        "category": "가전/IT",   "search_keyword": "에어프라이어"},
...
- 각 상품의 장점·단점을 쿠팡 구매자 리뷰 기반으로 정리
- "이런 분께 추천 / 이런 분은 패스" 형태로 구매 가이드 제공
- 절대 금지: "제가 직접 써봤는데", "사용해보니", "구매해서 써봤습니다" 등 1인칭 체험 표현
- 대신 사용: "쿠팡 평점 4.5 이상", "리뷰 1000개 이상인 인기 제품", "스펙 비교 결과"{"" if not products else chr(10) + "- 위 쿠팡 상품 데이터의 실제 제품명·가격·평점·리뷰 수를 가이드에 구체적으로 반영할 것"}
...
아래 JSON 형식으로만 응답하세요:
{{
  "seo_title": "검색 최적화 제목 (40~60자, '추천'·'비교'·'가이드'·'TOP' 등 키워드 포함)",
