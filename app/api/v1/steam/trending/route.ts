...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get recently added/updated games with high discounts
    // Trending = recent updates + high discount percentage + metacritic score
    ...
    // Sort by a combination of factors: recency, discount, and score
    const sortedData = (data || []).sort((a, b) => {
      const scoreA = (a.discount_percent || 0) * 0.5 + (a.metacritic_score || 0) * 0.5;
      const scoreB = (b.discount_percent || 0) * 0.5 + (b.metacritic_score || 0) * 0.5;
    ...
    return NextResponse.json(
      {
        ...
        meta: {
          ...
          limit: limit_value,
          description: "Trending games based on recency, discount, and ratings",
        },
      },
      {
        status: 200,
